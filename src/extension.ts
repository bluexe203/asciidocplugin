...
export function activate(context: vscode.ExtensionContext) {
	...
	const cssm = fs.readFileSync(path.join(context.extensionPath, 'mermaid.min.css'));
	const cssa = fs.readFileSync(path.join(context.extensionPath, 'asciidoctor.css'));
	const css : string = "" + cssm + cssa;
...
function CreatePreviewHtml(context: vscode.ExtensionContext, webViewPanel : vscode.WebviewPanel | null, js : string, css : string)
{
	...
	{
		const html = asciidoctor.convert(ConvertMermaid(content));
